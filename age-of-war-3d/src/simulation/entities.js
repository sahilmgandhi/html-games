// Pure-logic entity port of ../age-of-war/js/entities.js (no rendering, no DOM).
// Combat math runs in original pixel units; see config.js for the metre mapping.
//
// 3D additions: every entity gets a numeric `id` (mesh keying) and a `z` lane
// depth (visual only; gameplay distances use x/y as in the original).
...
export class Unit {
  constructor(x, y, side, ageIndex, unitIndex, upgradeTier, isHero, z = 0) {
    this.id = allocId();
    this.x = x;
    this.y = y;
    this.z = z;
