...
export function attachBattleView(game, sim, fx, opts = {}) {
  ...
  function update(dt) {
    ...
    // The skeletal cast arrives after first spawn: drop pre-cast meshes once
    // so syncMap recreates them from the live entities.
    if (!quatSwapped && getQuatTemplates()) {
      quatSwapped = true;
      for (const [id, w] of units) {
        if (w.ageIndex <= 2) { scene.remove(w.mesh); w.dispose(); units.delete(id); }
      }
    }
